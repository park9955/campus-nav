...
if("POST".equalsIgnoreCase(request.getMethod())){
 ...
 try{
  c=getConnection();
  ps=c.prepareStatement("REPLACE INTO locations VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
  ps.setString(1,request.getParameter("locationId"));
  ps.setString(2,request.getParameter("locationType"));
  ps.setString(3,request.getParameter("locationName"));
  ps.setString(4,request.getParameter("buildingName"));
  ps.setInt(5,Integer.parseInt(request.getParameter("floorNo")));
  ps.setDouble(6,Double.parseDouble(request.getParameter("latitude")));
  ps.setDouble(7,Double.parseDouble(request.getParameter("longitude")));
  String alt=request.getParameter("altitude"); if(alt==null||alt.isBlank())ps.setNull(8,Types.DECIMAL);else ps.setDouble(8,Double.parseDouble(alt));
  ps.setString(9,request.getParameter("detailLocation"));
  ps.setString(10,request.getParameter("entranceLocation"));
  ps.setBoolean(11,"1".equals(request.getParameter("accessible")));
  String pw=request.getParameter("passageWidthM"); if(pw==null||pw.isBlank())ps.setNull(12,Types.DECIMAL);else ps.setDouble(12,Double.parseDouble(pw));
  ps.setBoolean(13,"1".equals(request.getParameter("elevator")));
  ps.setBoolean(14,"1".equals(request.getParameter("stairs")));
  ps.setBoolean(15,"1".equals(request.getParameter("loadingAvailable")));
  ps.setString(16,request.getParameter("loadingSpace"));
  ps.setString(17,request.getParameter("managerName"));
  ps.executeUpdate();msg="위치정보가 저장되었습니다.";
...
<main class="container-xl pb-5">
  ...
  <section class="mt-4">
    <div class="section-title">
      <i class="bi bi-geo-alt text-info"></i> 위치 관리
    </div>
  </section>

  <section class="form-section">
    <h2 class="h5 mb-4">② 출발지·목표지 위치 등록</h2>
    ...
    <form method="post">
      <div class="form-row">
        <div class="form-group">
          ...
          <input name="locationId" value="LOC-S001" required>
        </div>
        <div class="form-group">
          <label>위치 구분</label>
      ...
      <div class="form-row">
        <div class="form-group">
          <label>층</label>
          <input name="floorNo" type="number" required>
        </div>
        <div class="form-group">
          <label>위도</label>
      ...
      <div class="form-row">
        <div class="form-group">
          <label>상세 위치</label>
          <input name="detailLocation" placeholder="301호 앞" required>
        </div>
        <div class="form-group">
          <label>출입구 위치</label>
          <input name="entranceLocation" placeholder="동쪽 출입구">
        </div>
        <div class="form-group">
          <label>출입 가능</label>
        ...
        <div class="form-group">
          <label>통행로 폭(m)</label>
          <input name="passageWidthM" type="number" step="0.1">
        </div>
      </div>
    ...
    <div class="table-responsive">
      <table class="table-air">
        ...
        <tbody>
          ...
          try{c2=getConnection();s2=c2.createStatement();r2=s2.executeQuery("SELECT * FROM locations ORDER BY location_id");
          while(r2.next()){%>
          <tr>
            <td><strong><%=r2.getString("location_id")%></strong></td>
            <td><%=r2.getString("location_type")%></td>
            <td><%=r2.getString("location_name")%></td>
            <td><%=r2.getString("building_name")%></td>
            <td><%=r2.getInt("floor_no")%></td>
            <td><small class="font-monospace"><%=r2.getDouble("latitude")%>, <%=r2.getDouble("longitude")%></small></td>
            <td><%=r2.getBoolean("elevator")?"있음":"없음"%></td>
            <td><%=r2.getBoolean("stairs")?"있음":"없음"%></td>
          </tr>
