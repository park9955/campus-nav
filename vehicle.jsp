...
if("POST".equalsIgnoreCase(request.getMethod())){
 ...
 try{
  c=getConnection(); ps=c.prepareStatement("REPLACE INTO vehicles VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
  ps.setString(1,request.getParameter("vehicleId"));
  ps.setDouble(2,Double.parseDouble(request.getParameter("currentLatitude")));
  ps.setDouble(3,Double.parseDouble(request.getParameter("currentLongitude")));
  ps.setDouble(4,Double.parseDouble(request.getParameter("payloadKg")));
  ps.setDouble(5,Double.parseDouble(request.getParameter("loadWidthM")));
  ps.setDouble(6,Double.parseDouble(request.getParameter("maxSpeedKmh")));
  ps.setDouble(7,Double.parseDouble(request.getParameter("batterySoc")));
  ps.setDouble(8,Double.parseDouble(request.getParameter("minBatterySoc")));
  ps.setDouble(9,Double.parseDouble(request.getParameter("turningRadiusM")));
  ps.setDouble(10,Double.parseDouble(request.getParameter("maxSlopeDeg")));
  ps.setBoolean(11,"1".equals(request.getParameter("indoorAvailable")));
  ps.setBoolean(12,"1".equals(request.getParameter("outdoorAvailable")));
  ps.setBoolean(13,"1".equals(request.getParameter("elevatorAvailable")));
  ps.setString(14,request.getParameter("status"));
  ps.executeUpdate();msg="운송체가 저장되었습니다.";
...
<main class="container-xl pb-5">
  ...
  <section class="mt-4">
    <div class="section-title">
      <i class="bi bi-truck text-info"></i> 운송체 관리
    </div>
  </section>

  <section class="form-section">
    <h2 class="h5 mb-4">③ 운송체 등록</h2>
    ...
    <form method="post">
      <div class="form-row">
        ...
        <div class="form-group">
          <label>현재 위도</label>
          <input name="currentLatitude" type="number" step="0.0000001" required>
        </div>
        <div class="form-group">
          <label>현재 경도</label>
          <input name="currentLongitude" type="number" step="0.0000001" required>
        </div>
        <div class="form-group">
          <label>적재 가능중량(kg)</label>
          <input name="payloadKg" type="number" step="0.1" required>
        </div>
      </div>
      ...
      <div class="form-row">
        <div class="form-group">
          <label>적재함 폭(m)</label>
          <input name="loadWidthM" type="number" step="0.1" required>
        </div>
        <div class="form-group">
          <label>최대속도(km/h)</label>
          <input name="maxSpeedKmh" type="number" step="0.1" required>
        </div>
        <div class="form-group">
          <label>현재 배터리(%)</label>
          <input name="batterySoc" type="number" value="78" required>
        </div>
        <div class="form-group">
          <label>최소 운행 배터리(%)</label>
          <input name="minBatterySoc" type="number" value="20" required>
        </div>
      </div>
      ...
      <div class="form-row">
        <div class="form-group">
          <label>회전반경(m)</label>
          <input name="turningRadiusM" type="number" step="0.1" required>
        </div>
        <div class="form-group">
          <label>경사 허용도(°)</label>
          <input name="maxSlopeDeg" type="number" step="0.1" required>
        </div>
        <div class="form-group">
          <label>실내운행</label>
          <select name="indoorAvailable">
    ...
    <div class="table-responsive">
      <table class="table-air">
        ...
        <tbody>
          ...
          try{c2=getConnection();s2=c2.createStatement();r2=s2.executeQuery("SELECT * FROM vehicles");
          while(r2.next()){%>
          <tr>
            <td><strong><%=r2.getString("vehicle_id")%></strong></td>
            <td><%=r2.getDouble("payload_kg")%>kg</td>
            <td><%=r2.getDouble("load_width_m")%>m</td>
            <td><%=r2.getDouble("battery_soc")%>%</td>
            <td><%=r2.getDouble("max_slope_deg")%>°</td>
            <td><%=r2.getBoolean("indoor_available")?"✓":"✗"%></td>
            <td><%=r2.getBoolean("outdoor_available")?"✓":"✗"%></td>
            <td>
              <%
                String status = r2.getString("status");
                String badgeClass = status.equals("대기") ? "status-ready" : "status-busy";
              %>
